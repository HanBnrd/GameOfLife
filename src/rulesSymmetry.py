...
class Rules:
    ...
    def getAll(self):
        ...
        for state in allStates:
            if np.array_equal(state,np.flip(state,0)):
                continue
            if any((np.flip(state,0)==x).all() for x in toRemove):
                continue
            for s in allStates:
                if np.array_equal(state,np.flip(s,0)):
                    toRemove.append(state)
        for rem in toRemove:
            index=0
            for state in allStates:
                if np.array_equal(rem,state):
                    allStates.pop(index)
                index+=1
        ...
        toRemove=[]
        for state in allStates:
            if np.array_equal(state,np.flip(state,1)):
                continue
            if any((np.flip(state,1)==x).all() for x in toRemove):
                continue
            for s in allStates:
                if np.array_equal(state,np.flip(s,1)):
                    toRemove.append(state)
        for rem in toRemove:
            index=0
            for state in allStates:
                if np.array_equal(rem,state):
                    allStates.pop(index)
                index+=1
        print("États restants : ",len(allStates))
